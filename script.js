...
// Page 0: Introduction Logic
const typingText = document.getElementById('typing-text');
const nameInput = document.getElementById('name-input');
const continueBtn = document.getElementById('continue-btn');
const validationMessage = document.getElementById('validation-message');
const identityConfirmation = document.getElementById('identity-confirmation');
const identityText = document.getElementById('identity-text');
const identityButtons = document.getElementById('identity-buttons');
const identityResponse = document.getElementById('identity-response');
...
const finalQuestionButtons = document.getElementById('final-question-buttons');
const finalQuestionResponse = document.getElementById('final-question-response');
...
// Initialize Page 0
function initPage0() {
    typeText('🚨 Birthday Verification Required', typingText, 80);
}
...
continueBtn.addEventListener('click', () => {
    ...
    if (name === '') {
        ...
        if (emptySubmitCount === 1) {
            validationMessage.textContent = "Really? You don't know your own name?";
        } else if (emptySubmitCount === 2) {
            validationMessage.textContent = "Interesting strategy... still waiting.";
        } else {
            validationMessage.textContent = "This is getting awkward. Please enter your name.";
        }
    ...
    if (name.length === 1) {
        validationMessage.textContent = "That's a name? Finish typing, lazy.";
        validationMessage.style.opacity = '1';
        nameInput.classList.add('shake');
        setTimeout(() => nameInput.classList.remove('shake'), 500);
        return;
    ...
    typingText.textContent = `Searching for "${userName}"...`;
    
    setTimeout(() => {
        typingText.textContent = 'Birthday human found.';
        setTimeout(() => {
...
function showIdentityConfirmation() {
    ...
    identityText.textContent = `So you're ${userName}?`;
    identityConfirmation.classList.add('show');
    
    identityButtons.innerHTML = '';
    
    const obviouslyBtn = document.createElement('button');
    obviouslyBtn.className = 'btn';
    obviouslyBtn.textContent = 'Obviously.';
    obviouslyBtn.addEventListener('click', () => {
        identityResponse.textContent = 'Confidence detected.';
        identityResponse.style.opacity = '1';
        setTimeout(() => {
            showFinalQuestion();
        }, 1500);
    });
    
    const notAnymoreBtn = document.createElement('button');
    notAnymoreBtn.className = 'btn';
    notAnymoreBtn.textContent = 'Not anymore.';
    notAnymoreBtn.addEventListener('click', () => {
        identityResponse.textContent = 'Identity crisis on your birthday? Bold choice.';
        identityResponse.style.opacity = '1';
        setTimeout(() => {
            showFinalQuestion();
        }, 1500);
    ...
    identityButtons.appendChild(obviouslyBtn);
    identityButtons.appendChild(notAnymoreBtn);
}

// Final question
function showFinalQuestion() {
    identityConfirmation.classList.remove('show');
    finalQuestionText.textContent = 'One last question...';
    finalQuestion.classList.add('show');
    
    setTimeout(() => {
        finalQuestionText.textContent = 'Are you actually ready for what\'s coming?';
        
        finalQuestionButtons.innerHTML = '';
        
        const bringItBtn = document.createElement('button');
        bringItBtn.className = 'btn';
        bringItBtn.textContent = 'Bring it on.';
        bringItBtn.addEventListener('click', () => {
            finalQuestionResponse.textContent = 'That\'s the spirit.';
            finalQuestionResponse.style.opacity = '1';
            setTimeout(() => {
                goToLandingPage();
            }, 1500);
        });
        ...
        scaredBtn.className = 'btn';
        scaredBtn.textContent = 'I\'m scared.';
        scaredBtn.addEventListener('click', () => {
            finalQuestionResponse.textContent = 'Good. You should be.';
            finalQuestionResponse.style.opacity = '1';
            setTimeout(() => {
                goToLandingPage();
            }, 1500);
        ...
        finalQuestionButtons.appendChild(bringItBtn);
        finalQuestionButtons.appendChild(scaredBtn);
    }, 1000);
}
...
// Update landing page title with name
function updateLandingTitle() {
    const landingTitle = document.getElementById('landing-title');
    landingTitle.textContent = `Hey ${userName}… today is your birthday 🎂`;
}
...
function updateFinalHeading() {
    const finalHeading = document.getElementById('final-heading');
    console.log('Updating final heading. userName:', userName);
    if (userName && userName.trim() !== '') {
        finalHeading.textContent = `🎉 HAPPY BIRTHDAY ${userName} 🎉`;
    } else {
        finalHeading.textContent = `🎉 HAPPY BIRTHDAY 🎉`;
    }
}
